    ...
    <PageComponent title="Doctor View Patient">
        ...
        <div v-else class="grid grid-cols-1 md:grid-cols-1 lg:grid-cols-1 gap-5 text-gray-700">
            <DashboardCard v-if="appointment.status == 0" class="order-1 lg:order-1 row-span-2 bg-blue-200"
                ...
                <div v-if="appointment.owner_type == 'Doctor' && appointment.owner_id == user.id" class="flex justify-between text-m mb-2">
                ...
                <div v-if="appointment.owner_type == 'Institute' && appointment.owner_id == user.instituition_id" class="flex justify-between text-m mb-2">
                    <button class="bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-4 rounded-full"
                        @click="cancelAppointment(appointment.id)">
                        Cancel
                    </button>
